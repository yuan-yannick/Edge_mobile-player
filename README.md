...
面向 YouTube 和部分结构简单的 HTML5 视频播放器：按住约 0.35 秒临时切换到 3 倍速，松开后恢复原速。

> 本项目没有针对抖音（Douyin）或 TikTok 做适配，也不保证在这些站点可用；它们已经提供原生长按倍速功能，建议直接使用网站或 App 自带功能。
...
- 部分结构简单、能直接访问 `<video>` 元素的 HTML5 播放器通常可用，但未逐站点保证兼容。
...
| 场景 | 支持情况 |
|---|---|
| YouTube 普通视频、Shorts | 已适配 |
| 页面中存在可见 `<video>` 的简单 HTML5 播放器 | 通常可用 |
| 结构复杂、持续接管倍速或无法访问底层视频的网站 | 不保证可用 |
| 抖音（Douyin）、TikTok | 未适配；请使用其原生长按倍速 |
| 抖音/TikTok App、其他原生应用 | 不可用；浏览器扩展无法注入 App |
| 图片、Canvas/WebGL 视频、系统画中画窗口 | 不可用或不保证可用 |
